...
The app's cross-platform target is **Linux, Windows, macOS, and Android** (Tauri v2 covers desktop and mobile — see [Scope/setup](../TODO.md)). Three responsibilities of the `Audio Engine`, three crates, chosen to prioritize the least possible friction running on all four platforms with no platform-specific code inside the module itself:
