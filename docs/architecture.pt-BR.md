...
O alvo multiplataforma do app é **Linux, Windows, macOS e Android** (Tauri v2 cobre desktop e mobile). Três responsabilidades do `Audio Engine`, três crates, escolhidas para priorizar o mínimo de atrito possível rodando nas quatro plataformas, sem código específico de plataforma dentro do próprio módulo:
