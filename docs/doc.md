...
**Rust é a base do projeto — a fonte da verdade do estado vive só ali, nunca na camada de apresentação.** Angular (na WebView do Tauri) é a escolha atual de apresentação, não uma premissa do núcleo: nenhum módulo do lado Rust deve depender de algo específico do Angular ou do Tauri pra tomar decisão de domínio. Essa separação é o que deixa aberta, em tese, a substituição da camada de apresentação inteira — por exemplo, por Flutter — sem tocar no núcleo; só a ponte de comunicação (e sua implementação de IPC) mudaria. Qualquer estado mantido do lado da apresentação (services + Signals, ver abaixo) é uma **projeção/cache** do que o Rust já decidiu, sincronizada via `Commands`/`Events` — nunca uma segunda fonte de verdade que possa divergir do núcleo.
...
O estado de `Gerenciador de Sessão / Estado`, `Gerenciador de Loops e Marcadores` e `Persistência de Projetos` é acessado por handlers de `Command` (rodando em tasks assíncronas, potencialmente concorrentes entre si) e precisa, em algum ponto, alimentar a thread de tempo real do `cpal`. A escolha aqui é deliberadamente o modelo **mais simples de raciocinar**, não o mais rápido em teoria: um único `Arc<Mutex<AppState>>` (ou um punhado de `Mutex`, um por módulo — não múltiplos por feature) guardando esse estado. Um handler de `Command` faz `lock()`, lê/muda o que precisa, solta o lock — igual à intuição de "só uma coisa por vez pode tocar nisso", sem precisar entender canais, actors ou ordenação de memória atômica pra trabalhar no núcleo.

Duas coisas tornam esse modelo simples viável sem virar gambiarra de performance:

- **A fila de `Commands` do lado Angular já limita a concorrência real** que chega no núcleo (ver [nota de design](#nota-de-design-fila-de-commands-no-lado-angular)) — não é um servidor atendendo milhares de requisições simultâneas, é uma pessoa clicando botões. Contenção de lock nesse volume é, na prática, não-mensurável.
...
#### Nota de design: fila de Commands no lado Angular

Todo `Command` disparado pela UI entra numa fila interna do lado Angular antes de ser enviado ao núcleo — mesmo sendo assíncrono. A fila existe para serializar o envio ao núcleo, não para impor espera perceptível ao usuário; são dois comportamentos distintos, fáceis de confundir:

- **Por botão/ação**: o controle que disparou o `Command` fica desabilitado desde o clique até chegar a notificação de sucesso ou erro *daquele* `Command` — evita duplo clique reenviando o mesmo comando (ex.: apertar "play" duas vezes gerando dois Commands de play).
- **Entre ações diferentes**: esse desabilitar é local à ação, não global. Outro botão pode disparar seu próprio `Command` enquanto o primeiro ainda está na fila ou em voo — os dois entram na mesma fila e são processados em sequência, cada um com sua própria notificação de sucesso/erro ao concluir.

Na prática, o usuário não deve perceber a fila — ela deve drenar rápido o suficiente pra parecer resposta imediata. Se o throughput se tornar um problema percebido (fila acumulando, resposta perceptivelmente atrasada), é sinal de otimizar a fila ou o núcleo — não de removê-la ou de travar a UI inteira enquanto ela processa.

Como o Rust é a fonte da verdade (ver [Stack e plataforma](#stack-e-plataforma)), a notificação de sucesso é o que confirma a mudança de estado — a UI não aplica a mudança de forma otimista antes da resposta do `Command`. O que o usuário vê refletir imediatamente é o botão desabilitado (feedback de "em andamento"), não o novo estado em si.
